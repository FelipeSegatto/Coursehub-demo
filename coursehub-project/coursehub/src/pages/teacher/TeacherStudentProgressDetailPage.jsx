...
import { getEnrollmentProgress } from "../../services/TeacherStudentProgressService";
...
/**
 * Mesmo detalhe da visão admin (AdminStudentProgressDetailPage) --
 * mesmos gráficos e mesmo PDF -- só que o backend já garante que a
 * matrícula pertence a uma turma/curso do professor autenticado.
 */
export default function TeacherStudentProgressDetailPage() {
  ...
  const fetchDetail = useCallback(async () => {
    ...
    try {
      ...
      const result = await getEnrollmentProgress(usuarioLogado.id, enrollmentId);

      setDetail(result || null);
    } catch (requestError) {
      console.error("[TeacherStudentProgressDetailPage] erro:", requestError);
      setError(requestError.message || "Não foi possível carregar o progresso desta matrícula.");
      setDetail(null);
    } finally {
      setLoading(false);
    }
  }, [usuarioLogado?.id, enrollmentId]);
      ...
      <main className="p-6">
        <div className="mx-auto max-w-3xl rounded-2xl border border-red-200 bg-red-50 p-6 text-center">
          ...
          <div className="mt-4 flex justify-center gap-3">
            ...
            <button
              type="button"
              onClick={() => navigate("/professor/progressao")}
    ...
    <main className="p-6">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mb-6 flex flex-col gap-4 rounded-2xl bg-white p-6 shadow md:flex-row md:items-start md:justify-between">
          ...
          <ExportPdfButton
            basePath={`/api/teacher/by-user/${usuarioLogado.id}/student-progress/enrollments/${enrollmentId}/export.pdf`}
            filters={{}}
            label="Exportar PDF"
          />
        </section>
